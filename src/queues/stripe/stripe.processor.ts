...
async function handlePaymentSuccess(event:Stripe.Event){
    try {
        ...
        const _stripeService = container.resolve<IStripeService>(Token.StripeService)
        const _onboardingRepository = container.resolve<IOnboardingRepository>(Token.OnboardingRepository)
        const _companyRepository = container.resolve<ICompanyRepository>(Token.CompanyRepository)
        ...
        console.log(userId)
        if(!onboarding) throw new CustomError(CONSTANT_MESSAGES.BAD_REQUEST,STATUS_CODES.BAD_REQUEST)
