...
import {
  IChangePlanDto,
  ICompleteOnboardingDto,
  ICompleteOnboardingPaymentDto,
  ICompleteOnboardingPaymentResDto,
  ICompleteOnboardingResDto,
  IGetOnboardingDto,
  IResponeOnboardingDto,
  ISaveOnboardingStepDto,
  IVerifyPaymentStatusDto,
  IVerifyPaymentStatusResDto,
} from "../../interfaces/dtos/OnboardingDto";
...
export class OnboardingService implements IOnboardingService {
  constructor(
    @inject(Token.OnboardingRepository)
    private readonly _onboardingRepository: IOnboardingRepository,
  ...
  async verifyPaymentStatus(
    ...
    try {
      ...
      const status = await this._paymentService.verifyPayment({
        sessionId: onboarding.sessionId ?? "",
      });
      if (!status) {
