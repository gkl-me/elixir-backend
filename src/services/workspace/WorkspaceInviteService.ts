...
export class WorkspaceInviteService implements IWorkspaceInviteService {
  constructor(
    @inject(Token.WorkspaceInviteRepository)
    private readonly _workspaceInviteRepository: IWorkspaceInviteRepository,
    ...
    try {
      ...
      return {
        invites: invites?.map(inv =>
          workspaceInviteDtoMapper.toListInvites(inv)
        ) ?? [],
  ...
  async revokeInvite(data: IRevokeInviteDto): Promise<void> {
    try {
      ...
      if (invite.status !== "pending") {
        throw new CustomError(
          "Cant only revoke pending invites",
          STATUS_CODES.BAD_REQUEST
        );
      }
  ...
  async validateInvite(
    data: IValidateInviteDto
  ): Promise<IValidateInviteResDto> {
    try {
      ...
      const workspace = await this._workspaceRepository.findById(
        invite.workspaceId
      );
      if (!workspace) {
        throw new CustomError(
