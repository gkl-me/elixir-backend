...
export class WorkspaceRoleService implements IWorkspaceRoleService {
    ...
    constructor(
        @inject(Token.WorkspaceRoleRepository) private readonly _workspaceRoleRepository: IWorkspaceRoleRepository
    ) { }
    ...
    async getRoles(data: IGetRolesDto): Promise<IGetRolesResDto[] | []> {
        try {
            ...
            return roles ? roles?.map((r) => workspaceRoleDtoMapper.toGetRoles(r)) : []
    ...
    async deleteRole(data: IDeleteRoleDto): Promise<void> {
        try {
            ...
            const memberRole = await this._workspaceRoleRepository.findOne({ workspaceId, key: 'member' })
