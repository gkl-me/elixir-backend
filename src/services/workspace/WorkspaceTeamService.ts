...
import { IAddMembersDto, ICreateTeamDto, IGetTeamDto, IGetTeamResDto, IListTeamsDto, IListTeamsResDto, IRemoveMemberDto } from "../../interfaces/dtos/WorkspaceTeamDto";
...
export class WorkspaceTeamService implements IWorkspaceTeamService {
    ...
    async createTeam(data: ICreateTeamDto): Promise<void> {
        try {
            ...
            const { workspaceId, name, description, memberIds, createdByUserId } = data;
    ...
    async getTeam(data: IGetTeamDto): Promise<IGetTeamResDto | null> {
        try {
            ...
            const teamMembers = await this._workspaceTeamRepository.listTeamMembers(workspaceId, teamId)
            if (!teamMembers) {
                throw new CustomError("Team not found", STATUS_CODES.BAD_REQUEST)
            }
