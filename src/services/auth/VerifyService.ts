...
export class VerifyService implements IVerifyService{
    ...
    async sendVerificationEmail(data: ISendVerificationEmailDto): Promise<void> {
        try {
            ...
            if(!userId) throw new CustomError(AUTH_MESSAGES.NOT_FOUND,STATUS_CODES.NOT_FOUND)
    ...
    async verifyEmail(data: IVerifyEmailDto,meta:IVerifyMetaDto): Promise<IAuthResponseDto> {
        try {
            ...
            const {token} = data
            if(!token) throw new CustomError(AUTH_MESSAGES.TOKEN_ERROR,STATUS_CODES.BAD_REQUEST)
