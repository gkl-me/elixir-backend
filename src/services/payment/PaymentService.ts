...
import { ICheckoutDto, IRetryPaymentDto, IVerifyPaymentDto } from "../../interfaces/dtos/PaymentDto";
...
export class PaymentService implements IPaymentService {
    ...
    async startCheckout(data:ICheckoutDto){
        try {

            const {userId,planId,sessionId} = data

            const user = await this._userRepository.findById(userId)
            if(!user) throw new CustomError(CONSTANT_MESSAGES.BAD_REQUEST,STATUS_CODES.BAD_REQUEST)

            const plan = await this._planRepository.findById(planId)
            if(!plan || !plan.stripePriceId) throw new CustomError(CONSTANT_MESSAGES.BAD_REQUEST,STATUS_CODES.BAD_REQUEST)
            
            

            if(!user.stripeCustomerId){
                const customerId = await this._stripeService.createCustomer(user.email,user.name,userId)
                if(!customerId) throw new CustomError(CONSTANT_MESSAGES.BAD_REQUEST,STATUS_CODES.BAD_REQUEST)
            ...
            const existingSubscription = await this._stripeService.getSubscription(user.stripeCustomerId)
    ...
    async verifyPayment(data:IVerifyPaymentDto){
        try {
    ...
    async retryPayment(data:IRetryPaymentDto){
        try {

            const {userId} = data

            const user = await this._userRepository.findById(userId)
            if(!user || !user.stripeCustomerId) throw new CustomError(CONSTANT_MESSAGES.BAD_REQUEST,STATUS_CODES.BAD_REQUEST)

            const invoice = await this._stripeService.getOpenInvoice(user.stripeCustomerId)
