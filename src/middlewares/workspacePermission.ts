...
export const requirePermission = (...required:WORKSPACE_PERMISSION[]) => {
    return async (req:Request,res:Response,next:NextFunction) => {
        try {
            ...
            const _workspaceMemberRepository = container.resolve<IWorkspaceMemberRepository>(Token.WorkspaceMemberRepository)
            const _workspaceRoleRepository = container.resolve<IWorkspaceRoleRepository>(Token.WorkspaceRoleRepository)
            ...
            const memberRepo = await _workspaceMemberRepository.findOne({workspaceId,userId,isRemoved:false})
            const roleRepo = await _workspaceRoleRepository.findById(memberRepo?.roleId || "")
