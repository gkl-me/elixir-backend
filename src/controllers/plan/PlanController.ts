...
export class PlanController implements IPlanController {
    ...
    async handleFindAllPlans(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const { plans, totalPage, currentPage } = await this._planService.findAllPlans(processedParams)
            successResponse(res, PLAN_MESSAGES.FETCH_SUCCESS, STATUS_CODES.OK, { plans, totalPage, currentPage })
        } catch (error) {
            next(error)
        }
    }

    async handleTogglePlanStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
