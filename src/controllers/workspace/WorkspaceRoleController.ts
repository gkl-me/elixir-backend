...
export class WorkspaceRoleController implements IWorkspaceRoleController {
    constructor(
        @inject(Token.WorkspaceRoleService) private readonly _workspaceRoleService: IWorkspaceRoleService
    ) { }

    async handleGetRoles(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
    ...
    async handleCreateRole(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            successResponse(res, "Workspace role created successfullly", STATUS_CODES.CREATED, {})
    ...
    async handleUpdateRole(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {

            const { workspaceId, roleId } = extractStringParams(req.params, ["workspaceId", "roleId"])

            //validate 
            const { name, permissions } = req.body

            const role = await this._workspaceRoleService.updateRole({
                roleId,
                workspaceId,
                name,
                permissions
            ...
            successResponse(res, "Workspace Role succesffuly updated", STATUS_CODES.OK, {})
    ...
    async handleDeleteRole(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
