...
export class PaymentController implements IPaymentController{
    ...
    async handleVerifyPayment(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {

            const userId = req.user.userId

            const {paymentStatus} = await this._onboardingService.verifyPaymentStatus({userId})
    ...
    async handleRetryPayment(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {

            const userId = req.user.userId

            const {payment_url} = await this._paymentService.retryPayment({userId})
