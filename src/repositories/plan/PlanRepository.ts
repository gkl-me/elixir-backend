...
export class PlanRepository extends BaseRepository<IPlan> implements IPlanRepository{
    ...
    async updateMany(filter: FilterQuery<IPlan>, update: UpdateQuery<IPlan>): Promise<UpdateWriteOpResult> {
        try {
            ...
            throw new CustomError(`Failed to update many ${this._model.modelName}`,STATUS_CODES.INTERNAL_SERVER_ERROR)
        }
    }
}